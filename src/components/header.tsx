...
const ThemeToggle = dynamic(() => import('./themeToggle'), {
  ssr: false,
  loading: () => (
    <div className="border border-slate-700 shadow-sm rounded-md h-8 w-8 ">
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 backdrop-blur-lg ">
      <div className="w-auto py-2 px-4 h-16 flex flex-row justify-between items-center max-w-3xl mx-auto">
        <Link
          href="/"
          aria-label="Ilyes Benkhelifa, home"
        ...
        <div className="flex items-center">
          <nav
            aria-label="Main"
            className="hidden md:flex items-center font-bold justify-end uppercase"
          >
            <Link
              href="/about"
            ...
            <Link
              ...
              className={`px-4 hover:text-accent ${
                pathname.startsWith('/projects') ? 'text-accent' : 'text-text'
              }`}
            >
              Projects
            </Link>
            ...
            >
              ...
              <span className="h-10 w-10 flex m-auto" aria-hidden="true">
                <MainGithubIcon />
              </span>
            </Link>
          </nav>
